...
def write_dsp_framework_output(template_path, output_path, results, ctx):
    wb = load_workbook(template_path, keep_vba=True)
    ws_main = wb['Framework_Analysis']
    ws_ref  = wb['Framework_Reference']

    # ── Analysis tab header ───────────────────────────────────────────────
    ws_main['A1'] = f"{ctx.hash_name} — Framework Analysis"
    ...
    for cid, res in results.items():
        if cid not in cid_to_row:
            print(f'[writer_dsp_framework] WARNING: {cid} not found in reference sheet — skipping.')
            continue
        rr = cid_to_row[cid]
